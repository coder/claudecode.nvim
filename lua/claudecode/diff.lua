...
---@param terminal_win_in_new_tab number? Terminal window in new tab if created
---@param existing_buffer number? Existing buffer for the file if already loaded
...
function M._create_diff_view_from_window(
  ...
  local need_new_window_for_original = false

  -- Determine if we need to create a new window for the original file
  local in_new_tab = terminal_win_in_new_tab ~= nil

  local current_buf = vim.api.nvim_win_get_buf(target_window)
  local current_buf_path = vim.api.nvim_buf_get_name(current_buf)

  if in_new_tab then
    -- In a new tab, always use the target_window (main area) for the original file
    -- This prevents creating extra splits in new tabs
    original_window = target_window
    need_new_window_for_original = false
  elseif is_new_file then
    -- For new files (not in new tab), check if we can reuse an empty buffer in target_window
    local is_empty_buffer = current_buf_path == "" and vim.api.nvim_buf_get_option(current_buf, "modified") == false

    if is_empty_buffer then
      original_window = target_window
      original_buffer = current_buf
      need_new_window_for_original = false
    else
      -- The target window has content - don't disturb it, create a new window
      need_new_window_for_original = true
    end
  else
    -- For existing files (not in new tab), check if target_window already has the file we want
    -- Check if we can reuse an empty buffer for existing files too
    local is_empty_buffer = current_buf_path == "" and vim.api.nvim_buf_get_option(current_buf, "modified") == false
    ...
    elseif is_empty_buffer then
      -- We can reuse this empty buffer for the existing file
      original_window = target_window
      original_buffer = current_buf
      need_new_window_for_original = false
    else
      -- The target window has a different buffer - don't disturb it!
      need_new_window_for_original = true
    end
  end

  -- Handle the case where we're using the target window directly (new tab scenario)
  if not need_new_window_for_original and in_new_tab then
    if is_new_file then
      -- Create and setup empty buffer for new file in new tab
      ...
      if existing_buffer then
        vim.api.nvim_win_set_buf(original_window, existing_buffer)
        original_buffer = existing_buffer
      else
        vim.cmd("edit " .. vim.fn.fnameescape(old_file_path))
        original_buffer = vim.api.nvim_win_get_buf(original_window)
      end
    end
  end

  -- Create a new window for the original file if needed (non-new-tab scenario)
  if need_new_window_for_original then
    vim.api.nvim_set_current_win(target_window)
    create_split()
    original_window = vim.api.nvim_get_current_win()

    if is_new_file then
      -- Create and setup empty buffer for new file
      local empty_buffer = vim.api.nvim_create_buf(false, true)
      if not empty_buffer or empty_buffer == 0 then
        local error_msg = "Failed to create empty buffer for new file diff"
        logger.error("diff", error_msg)
  ...
  -- If we reused an empty buffer for an existing file, we need to load the file now
  if not need_new_window_for_original and not in_new_tab and not is_new_file and original_buffer then
    -- We reused an empty buffer - need to load the existing file into it
    local reused_buf_path = vim.api.nvim_buf_get_name(original_buffer)
    if reused_buf_path == "" then
  ...
  -- Set current window to the one with the original file
  vim.api.nvim_set_current_win(original_window)
  ...
  -- No need to restore buffers anymore since we never replace them!
  ...
  if config and config.diff_opts and config.diff_opts.keep_terminal_focus then
    vim.schedule(function()
      -- Check if we're in a new tab with a terminal window
      if terminal_win_in_new_tab and vim.api.nvim_win_is_valid(terminal_win_in_new_tab) then
        -- Focus the terminal window in the new tab
        vim.api.nvim_set_current_win(terminal_win_in_new_tab)
        -- Enter terminal mode (insert mode for terminal buffers)
        vim.cmd("startinsert")
        return
      end

      -- Otherwise use existing logic for same-tab terminal focus
  ...
  -- If we're in a new tab with a terminal, restore the terminal width
  -- (creating splits can mess up the terminal width configuration)
  if terminal_win_in_new_tab and vim.api.nvim_win_is_valid(terminal_win_in_new_tab) then
    local terminal_config = config.terminal or {}
    local split_width = terminal_config.split_width_percentage or 0.30
    local total_width = vim.o.columns
