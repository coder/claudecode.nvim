...
function M._create_autocommands()
  local group = vim.api.nvim_create_augroup("OpenCodeSelection", { clear = true })
...
function M._clear_autocommands()
  vim.api.nvim_clear_autocmds({ group = "OpenCodeSelection" })
end
...
function M.send_current_selection()
  if not M.state.tracking_enabled or not M.server then
    logger.error("selection", "OpenCode is not running")
    return
  end
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  -- Check if OpenCode integration is running (server may or may not have clients)
  local opencode_main = require("opencode")

  if not using_opencode and not opencode_main.state.server then
    logger.error("selection", "OpenCode integration is not running.")
    return false
  end
  ...
  local success, error_msg = opencode_main.send_at_mention(file_path, start_line, end_line, "OpenCodeSend")
