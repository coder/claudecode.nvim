--- Diff module for OpenCode Neovim integration.
...
local function get_autocmd_group()
  if not autocmd_group then
    autocmd_group = vim.api.nvim_create_augroup("OpenCodeMCPDiff", { clear = true })
  end
...
---Find the OpenCode terminal window to keep focus there.
---Uses the terminal provider to get the active terminal buffer, then finds its window.
---@return number? win_id Window ID of the OpenCode terminal window, or nil if not found
local function find_opencode_terminal_window()
...
---Display existing OpenCode terminal in new tab
...
local function display_terminal_in_new_tab()
  ...
  local existing_terminal_win = find_opencode_terminal_window()
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    desc = "Auto-enter terminal mode when focusing OpenCode terminal",
  })
...
---@param user_config OpenCodeConfig The configuration passed from init.lua
...
local function create_temp_file(content, filename)
  local base_dir_cache = vim.fn.stdpath("cache") .. "/opencode_diffs"
  ...
  if mkdir_ok_cache then
    ...
    local base_dir_temp = vim.fn.stdpath("cache") .. "/opencode_diffs_fallback"
...
local function cleanup_temp_file(tmp_file)
  if tmp_file and vim.fn.filereadable(tmp_file) == 1 then
    ...
    if vim.fs and type(vim.fs.remove) == "function" then
      ...
      if not ok_file then
        vim.notify(
          "OpenCode: Error removing temp file " .. tmp_file .. ": " .. tostring(err_file),
          vim.log.levels.WARN
        )
      end
      ...
      if not ok_dir then
        vim.notify(
          "OpenCode: Error removing temp directory " .. tmp_dir .. ": " .. tostring(err_dir),
          vim.log.levels.INFO
        )
      end
    else
      ...
      vim.notify(
        "OpenCode: Cannot perform standard cleanup: "
          .. reason
          .. ". Affected file: "
          .. tmp_file
      ...
      if not os_ok then
        vim.notify(
          "OpenCode: Fallback os.remove also failed for file " .. tmp_file .. ": " .. tostring(os_err),
          vim.log.levels.ERROR
        )
      end
    end
...
local function setup_new_buffer(
  ...
  vim.b[new_buf].opencode_diff_tab_name = tab_name
  vim.b[new_buf].opencode_diff_new_win = new_win
  vim.b[new_buf].opencode_diff_target_win = target_win_for_meta
  ...
  if config and config.diff_opts and config.diff_opts.keep_terminal_focus then
    vim.schedule(function()
      ...
      local terminal_win = find_opencode_terminal_window()
...
function M.accept_current_diff()
  ...
  local tab_name = vim.b[current_buffer].opencode_diff_tab_name
